...
// Debug setup
const ownName = 'cmdo'
const debug = require('ebug')(ownName)
...
function init (pj, helpStyle) {
  ...
  obj.parseTypedValue = function parseTypedValue (val, type) {
    let result
  ...
  obj.getNextOption = function nextOption () {
    ...
    //  Starting position of tokens removed
    let start = 0 // eslint-disable-line
    ...
    if (/^-(([a-z0-9]+)=*[.\w-]*[a-z0-9]+.*)*[a-z0-9.]+$/i.test(op)) {
      ...
      if (op.indexOf('=') !== -1) {
        ...
        throw new Error(`Don't recognize token format ${this.args[ 0 ]}`)
      }

      if (typeof val === 'undefined') {
        if (this.parsing[this.shorts[ op ]][ 2 ] !== 'boolean') {
          val = this.parseTypedValue(this.args[ 1 ], this.parsing[this.shorts[ op ]][ 2 ])
          count++
        } else {
          if (/^true|false$/i.test(this.args[ 1 ])) {
            // If the next token is a boolean value, parse it.
            val = this.args[ 1 ] === 'true'
            count++
          } else {
      ...
      if (typeof val === 'undefined') {
        ...
        if (this.parsing[this.shorts[this.longs[ op ]]][ 2 ] !== 'boolean') {
          val = this.parseTypedValue(this.args[ 1 ], this.parsing[this.shorts[this.longs[ op ]]][ 2 ])
          count++
        } else {
          if (/^true|false$/i.test(this.args[ 1 ])) {
            // If the next token is a boolean value, parse it.
    ...
    if (count > 0) {
      let parsed = this.args.splice(0, count) // eslint-disable-line

      // debug('Parsed token(s): %s', parsed.join(', '))
    }

    if (typeof this.shorts[ op ] === 'undefined' && typeof this.longs[ op ] === 'undefined') {
      throw new Error('Unknown option %s in args %s', op, process.argv.slice(2).join(', '))
    }
